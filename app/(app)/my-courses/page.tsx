    ...
    async function fetchData() {
      try {
        ...
        if (studentRes.ok) {
          ...
          setCreditLimit(student.creditLimit);
        }
      } catch {
        // ignore
      }
    ...
    <>
      ...
      <main className="flex-1 p-6 md:p-8 mx-auto w-full" style={{ maxWidth: "var(--spacing-container_max)" }}>
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--color-on-surface)" }}>My Registered Courses</h1>
            <p className="text-base" style={{ color: "var(--color-on-surface-variant)" }}>View and manage your current academic enrollment.</p>
          </div>
          <div className="flex items-center gap-2 px-4 py-3 rounded-lg border self-start md:self-auto"
            style={{ backgroundColor: "var(--color-surface-container-low)", borderColor: "var(--color-surface-variant)" }}>
            ...
            <span className="text-sm font-medium" style={{ color: "var(--color-on-surface)" }}>
              Total Credits: <strong style={{ color: "var(--color-tertiary)" }}>{totalCredits}</strong> / {creditLimit}
            </span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20" style={{ color: "var(--color-on-surface-variant)" }}>
            <span className="material-symbols-outlined text-6xl opacity-30 block mb-3">school</span>
            ...
            {filteredCourses.map((course) => {
              ...
              return (
                <div key={course.id} className="rounded-xl p-6 flex flex-col card-shadow border transition-all duration-300"
                  style={{ backgroundColor: "var(--color-surface)", borderColor: "transparent" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "var(--color-surface-variant)"; }}
                  ...
                  <div className="flex flex-col gap-2 mt-auto mb-4">
                    {[
                      ...
                      <div key={row.label} className="flex justify-between items-center py-1.5 border-b last:border-none"
                        style={{ borderColor: "var(--color-surface-container-highest)" }}>
                        <span className="text-xs" style={{ color: "var(--color-outline)" }}>{row.label}</span>
                        <span className="text-sm font-medium" style={{ color: "var(--color-on-surface)" }}>{row.value}</span>
                      </div>
                    ))}
                  </div>
                  <div className="mt-2 pt-4 border-t" style={{ borderColor: "var(--color-surface-container-high)" }}>
                    <button className="w-full border py-2 rounded-lg text-sm font-medium transition-all"
                      style={{ backgroundColor: "transparent", borderColor: "var(--color-primary)", color: "var(--color-primary)" }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = "var(--color-primary-fixed)"; (e.currentTarget as HTMLButtonElement).style.color = "var(--color-on-primary-fixed)"; }}
                      onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = "transparent"; (e.currentTarget as HTMLButtonElement).style.color = "var(--color-primary)"; }}>
                      View Details
                    </button>
                  </div>
                </div>
