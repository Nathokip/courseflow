...
import { droppableCourses, Course } from "@/lib/mock-data";

export default function DropCoursePage() {
  const [courses, setCourses] = useState<Course[]>(droppableCourses);
  ...
  return (
    <>
      ...
      <main
        className="flex-1 p-6 md:p-8 mx-auto w-full relative"
      ...
      >
        ...
        <div className="mb-8">
          ...
          >
            Select an active enrollment below to initiate the drop procedure.
            Please be aware that dropping courses mid-semester may affect your
            academic standing. Proceed with caution.
          </p>
        </div>
        ...
        {dropped.length > 0 && (
          ...
          >
            ...
            <span className="text-sm font-medium">
              {dropped.length} course{dropped.length > 1 ? "s" : ""} successfully
              dropped.
            </span>
          </div>
        )}
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <div
                key={course.id}
                className="rounded-xl p-6 flex flex-col relative overflow-hidden group transition-shadow card-shadow hover:card-shadow-md"
                style={{
      ...
      {pendingDrop && (
        ...
        >
          <div
            className="w-full max-w-lg rounded-2xl p-8 flex flex-col relative"
            style={{
              backgroundColor: "var(--color-surface-container-lowest)",
              boxShadow:
          ...
          >
            ...
            >
              <p
                ...
                style={{ color: "var(--color-on-surface)" }}
              >
                You are about to permanently drop the following course:
              </p>
              <p
                className="text-lg font-bold"
                style={{ color: "var(--color-on-surface)" }}
              >
                {pendingDrop.name} ({pendingDrop.code})
              </p>
            </div>
            ...
            <p
              ...
              style={{ color: "var(--color-on-surface-variant)" }}
            >
              Dropping this course will remove{" "}
              <strong style={{ color: "var(--color-on-surface)" }}>
                {pendingDrop.credits} credits
              </strong>{" "}
              from your current semester total. This action cannot be undone
              and may drop you below the full-time enrollment requirement.
            </p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={confirmDrop}
                className="px-6 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-opacity"
                style={{
