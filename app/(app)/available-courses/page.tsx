...
export default function AvailableCoursesPage() {
  ...
  const filtered = courses.filter((c) => {
    const matchFilter =
      ...
      (activeFilter === "Semester 1" ? c.semester === "Sem 1" : false) ||
      (activeFilter === "Semester 2" ? c.semester === "Sem 2" : false) ||
  ...
  return (
    <>
      ...
      <main className="flex-1 p-6 md:p-8 mx-auto w-full" style={{ maxWidth: "var(--spacing-container_max)" }}>
        ...
        <div className="flex md:hidden items-center rounded-full px-4 py-2 gap-2 border mb-4"
          style={{ backgroundColor: "var(--color-surface-container-low)", borderColor: "var(--color-outline-variant)" }}>
          <span className="material-symbols-outlined text-[20px]" style={{ color: "var(--color-outline)" }}>search</span>
          <input type="text" placeholder="Search courses..." value={search} onChange={(e) => setSearch(e.target.value)}
            className="bg-transparent border-none outline-none flex-1 text-sm" style={{ color: "var(--color-on-surface)" }} />
        </div>
          ...
          {departments.map((dep) => {
            ...
            return (
              <button key={dep} onClick={() => setActiveFilter(dep)}
                ...
                style={active ? { backgroundColor: "var(--color-primary-container)", color: "var(--color-on-primary-container)", border: "1px solid var(--color-primary)" } : { backgroundColor: "var(--color-surface)", color: "var(--color-on-surface-variant)", border: "1px solid var(--color-outline-variant)" }}>
